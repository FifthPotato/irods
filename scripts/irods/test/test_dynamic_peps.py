    ...
    def test_if_StructFileExtAndRegInp_is_exposed__issue_7413(self):
        with temporary_core_file() as core:
            ...
            pep_api_struct_file_ext_and_reg_post(*INSTANCE, *COMM, *STRUCTINP) {{
                ...
                *w = *STRUCTINP.oprType;
                msiModAVUMetadata('-C', '{self.user.session_collection}', 'add', '{avu_prefix}_2', *w, '');
            }}
            '''.format(**locals())))
            ...
            try:
                ...
                for x in range(0, 5):
                    self.user.assert_icommand(['itouch', "%s/file%s" % (TEST_COLLECTION_NAME, x)], 'STDOUT')

                self.user.assert_icommand(['ibun', '-cDtar', "%s/%s" % (self.user.session_collection, 'test.tar'), TEST_COLLECTION_NAME], 'STDOUT')

                self.user.assert_icommand(['imkdir', TEST_COLLECTION_NAME + "_backup"], 'STDOUT')

                # Should trigger PEP
                self.user.assert_icommand(['ibun', '-x', TEST_TARFILE_NAME, TEST_COLLECTION_NAME + "_backup"], 'STDOUT')

                # Show the session collection has the expected AVUs attached to it.
                self.user.assert_icommand(['imeta', 'ls', '-C', self.user.session_collection], 'STDOUT', [
                    f'attribute: {avu_prefix}_0\nvalue: {TEST_TARFILE_NAME}\n',
#                    f'attribute: {avu_prefix}_1\nvalue: {TEST_COLLECTION_NAME}\n',
                    f'attribute: {avu_prefix}_2\nvalue: 0\n',
                ])
                ...
                # Cleanup
                self.user.run_icommand(['irm', '-r', TEST_COLLECTION_NAME])
                self.user.run_icommand(['irm', '-r', TEST_COLLECTION_NAME + "_backup"])
                self.user.run_icommand(['irm', TEST_TARFILE_NAME])
